...
<div class="p-6 md:p-8">
    <div class="mb-8">
        ...
        <p class="text-muted-foreground">
            Resumen de tu blog y contenido
        </p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="shadcn-card rounded-xl border bg-card border-card-border text-card-foreground shadow-sm">
            ...
            <div class="p-6 pt-0">
                ...
                <p class="text-xs text-muted-foreground mt-1">
                    {{ $publishedPosts->count() }} publicados, {{ $draftPosts->count() }} borradores
                </p>
            </div>
        </div>
        ...
        <div class="shadcn-card rounded-xl border bg-card border-card-border text-card-foreground shadow-sm">
            <div class="flex flex-col space-y-1.5 p-6">
                <div class="flex items-center justify-between">
                    <h3 class="text-sm font-medium">Vistas Totales</h3>
                    <svg class="w-6 h-6 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="shadcn-card rounded-xl border bg-card border-card-border text-card-foreground shadow-sm">
        <div class="flex flex-col space-y-1.5 p-6">
            <h2 class="text-lg font-semibold leading-none tracking-tight">Posts Recientes</h2>
        </div>
        <div class="p-6 pt-0">
            @if($posts->count() > 0)
                <div class="space-y-4">
                    ...
                    >
                        <div class="flex-1">
                            ...
                            <p class="text-sm text-muted-foreground">
                                {{ $post->category->name ?? 'Sin categoría' }} • {{ $post->view_count }} vistas
                            </p>
                        </div>
