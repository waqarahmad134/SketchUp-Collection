...
<div class="p-6 md:p-8">
    ...
    <div class="shadcn-card rounded-xl border bg-card border-card-border text-card-foreground shadow-sm p-6 max-w-3xl">
        <form method="POST" action="{{ route('newadmin.transactions.update', $transaction->id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div>
                    ...
                    >
                    <p class="mt-1 text-xs text-muted-foreground">Transaction ID cannot be changed</p>
                </div>
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($users as $user)
                                <option value="{{ $user->id }}" {{ old('user_id', $transaction->user_id) == $user->id ? 'selected' : '' }}>{{ $user->name }} ({{ $user->email }})</option>
                            @endforeach
                        </select>
                        @error('user_id')
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($orders as $order)
                                <option value="{{ $order->id }}" {{ old('order_id', $transaction->order_id) == $order->id ? 'selected' : '' }}>{{ $order->order_number }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            value="{{ old('payment_gateway', $transaction->payment_gateway) }}"
                            class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                        >
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input
                        ...
                        value="{{ old('gateway_transaction_id', $transaction->gateway_transaction_id) }}"
                        class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                    >
                </div>
                ...
                <div>
                    ...
                    <input
                        ...
                        id="completed_at"
                        name="completed_at"
                        value="{{ old('completed_at', $transaction->completed_at ? $transaction->completed_at->format('Y-m-d\TH:i') : '') }}"
                        class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                    >
                </div>
                ...
                <div class="flex gap-3 pt-4">
                    ...
                    <a href="{{ route('newadmin.transactions.index') }}">
                        <button
                            type="button"
