...
<div class="p-6 md:p-8">
    <div class="flex items-center justify-between mb-8">
        ...
        <a href="{{ route('newadmin.users.create') }}">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b">
                            ...
                            <th class="text-left p-3 font-semibold">Verified</th>
                            <th class="text-left p-3 font-semibold">Orders</th>
                            <th class="text-left p-3 font-semibold">Posts</th>
                        ...
                        <tr class="border-b hover:bg-muted/50">
                            ...
                            <td class="p-3">
                                <span class="px-2 py-1 rounded text-xs font-medium
                                    ...
                                    @elseif($user->role === 'seller') bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200
                            ...
                            <td class="p-3">
                                @if($user->email_verified_at)
                                    <span class="px-2 py-1 rounded text-xs bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200">Yes</span>
                                @else
                                    <span class="px-2 py-1 rounded text-xs bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-200">No</span>
                                @endif
                            </td>
                            <td class="p-3">{{ $user->orders_count }}</td>
                            <td class="p-3">{{ $user->posts_count }}</td>
                            <td class="p-3 text-sm text-muted-foreground">{{ $user->created_at->format('d/m/Y') }}</td>
                            <td class="p-3">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('newadmin.users.edit', $user->id) }}">
                                    ...
                                    @if($user->id !== auth()->id())
                                    <form method="POST" action="{{ route('newadmin.users.destroy', $user->id) }}" onsubmit="return confirm('Are you sure? This action cannot be undone.');" class="inline">
                                        @csrf
                                        @method('DELETE')
