...
@section('title', 'New Custom Script')
...
<div class="p-6 md:p-8">
    ...
    <div class="shadcn-card rounded-xl border bg-card border-card-border text-card-foreground shadow-sm p-6 max-w-3xl">
        <form method="POST" action="{{ route('newadmin.custom-scripts.store') }}">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select
                        ...
                        class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                    >
                        <option value="head" {{ old('position', 'head') === 'head' ? 'selected' : '' }}>Head (Before &lt;/head&gt;)</option>
                        <option value="body_start" {{ old('position') === 'body_start' ? 'selected' : '' }}>Body Start (After &lt;body&gt;)</option>
                        <option value="body_end" {{ old('position') === 'body_end' ? 'selected' : '' }}>Body End (Before &lt;/body&gt;)</option>
                    </select>
                ...
                <div class="flex gap-3 pt-4">
                    ...
                    <a href="{{ route('newadmin.custom-scripts.index') }}">
                        <button
                            type="button"
